...
function getGreetingIndex() {
  ...
  if (raw !== null) {
    try {
      ...
      if (fresh && Number.isInteger(idx) && idx >= 0 && idx < GREETINGS.length)
        return idx;
    } catch {
  ...
  sessionStorage.setItem(
    ...
    JSON.stringify({ idx, pickedAt: Date.now() }),
  );
  return idx;
}
...
function Cover({ book, className = "" }: { book: Book; className?: string }) {
  ...
  return (
    <div
      className={`rounded-md bg-surface2 border border-border flex items-center justify-center p-2.5 ${className}`}
    >
      <span className="text-[11px] font-semibold leading-snug text-center text-ink/70">
        {book.title}
      </span>
    </div>
  );
...
export default function HomeTab({ state, userName, onFinishBook }: Props) {
  ...
  const doneThisYear = books.filter((b) =>
    b.finishedAt?.startsWith(String(thisYear)),
  ).length;
  const doneThisMonth = books.filter((b) =>
    b.finishedAt?.startsWith(
      `${thisYear}-${String(now.getMonth() + 1).padStart(2, "0")}`,
    ),
  ).length;
  const pct = readingGoal
    ? Math.min(100, Math.round((doneThisYear / readingGoal) * 100))
    : 0;
  ...
  const todayQuote: Quote | undefined = quotes.length
    ? quotes[quoteIdx % quotes.length]
    : undefined;
  const quoteBook = todayQuote
    ? books.find((b) => b.id === todayQuote.bookId)
    : undefined;
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {/* 목표 + 보조 지표 */}
      <div
        className={`${CARD} px-5 py-5 flex flex-col sm:flex-row sm:items-center gap-5 sm:gap-8`}
      >
                  ...
                  <div className="flex-1 min-w-0 h-[120px] flex flex-col">
                    <div className="flex items-start gap-3">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="mt-0.5 text-xs text-dim truncate">
                          {book.author || "저자 미상"}
                        </div>
                      </div>
                      <span className="mt-1 font-mono text-[11px] text-dim whitespace-nowrap flex-shrink-0">
                        {daysSince(readingSince(book))}일째 · 문장{" "}
                        {quotes.filter((q) => q.bookId === book.id).length}개
                      </span>
                    </div>
        ...
        <div className={`${CARD} px-5 py-4 flex flex-col gap-3`}>
          ...
          {todayQuote ? (
              ...
              <div className="flex-1 flex items-center py-2">
                <div className="font-serif text-sm sm:text-base leading-[1.85] text-ink">
                  ...
                  <HighlightedText
                    ...
                    highlights={todayQuote.highlights}
                  />
                  &rdquo;
                </div>
              </div>
            ...
            <div className="flex-1 flex flex-col justify-center gap-2 py-2">
              <span className="text-[13px] text-dim leading-relaxed">
                아직 모은 문장이 없어요
              </span>
              <button
          ...
          {activity.length === 0 ? (
            ...
            <div className="flex flex-col">
              {activity.map((a, i) => (
                <div
                  key={i}
                  className="flex items-center gap-3 py-2.5 border-b border-surface2 last:border-b-0"
                >
                  <span
                  ...
                  {a.kind === "finished" && a.rating ? (
                    <span className="text-[11px] text-dim tracking-[1px] flex-shrink-0">
                      {"★".repeat(a.rating)}
                    </span>
                  ) : null}
