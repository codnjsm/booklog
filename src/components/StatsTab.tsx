...
export default function StatsTab({ books, quotes, goal, onSetGoal }: Props) {
  ...
  for (let i = 11; i >= 0; i--) {
    ...
    if (b.finishedAt) {
      const k = b.finishedAt.slice(0, 7)
  ...
  quotes.forEach((q) => {
    if (q.bookId) quotesPerBook[q.bookId] = (quotesPerBook[q.bookId] || 0) + 1
  })
            ...
            {goal ? (
              <div>
                ...
                <div className="h-2 bg-surface2 rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-[width] duration-[400ms] ease-in-out ${'bg-accent'}`}
                    style={{ width: `${pct}%` }}
                  />
                </div>
              </div>
          ...
          {Object.entries(monthCounts).map(([key, v]) => {
            ...
            return (
              <div
                ...
                title={`${parseInt(key.split('-')[1])}월 ${v}권`}
              >
                <div className="flex-1 min-h-0 w-full flex flex-col justify-end items-center gap-2">
                  <div className="h-4 text-[11px] font-semibold text-ink [font-variant-numeric:tabular-nums]">
                    {v > 0 ? v : ''}
                  </div>
                  {/* 숫자 줄(1rem)과 간격(0.5rem)을 뺀 나머지 높이를 권수 비율로 나눠 갖는다 */}
                  <div
                    className="w-full rounded-t-[4px] min-h-[3px] transition-all duration-300 hover:opacity-80"
                    style={{
                ...
                <div className={`text-[10px] whitespace-nowrap ${isCurrent ? 'font-bold text-ink' : 'text-dim'}`}>
                  ...
                  <span className="max-[370px]:hidden">월</span>
                </div>
              </div>
            )
          })}
        ...
        <div className="bg-surface border border-border rounded-[10px] p-4 sm:p-[22px] mb-4">
          ...
          <div className="flex flex-col gap-2">
            {topAuthors.map(([a, c]) => (
              <div
                key={a}
                className="flex justify-between items-center py-[7px] px-2.5 sm:py-2 sm:px-3 bg-bg rounded-md text-[13px]"
              >
                <span>{a}</span>
                <span className="font-semibold [font-variant-numeric:tabular-nums]">{c}권</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-surface border border-border rounded-[10px] p-4 sm:p-[22px] mb-4">
          ...
          <div className="flex flex-col gap-2">
            {topQuoted.map((t) => (
              <div
                key={t.title}
                className="flex justify-between items-center gap-3 py-[7px] px-2.5 sm:py-2 sm:px-3 bg-bg rounded-md text-[13px]"
              >
                <span className="truncate">{t.title}</span>
                <span className="font-semibold flex-shrink-0 [font-variant-numeric:tabular-nums]">{t.count}개</span>
              </div>
            ))}
          </div>
        </div>
