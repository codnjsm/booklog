    ...
    >
      <div className="w-[90px] sm:w-full flex-shrink-0 sm:flex-shrink aspect-[2/3] bg-gradient-to-br from-surface2 to-bg flex items-center justify-center overflow-hidden relative rounded-none">
        {book.cover && !imgError ? (
          ...
          <div className="text-center p-4 font-sans">
            <span className="sm:hidden text-dim"><IconBooks size={26} /></span>
      ...
      <div className="flex-1 sm:flex-none px-3.5 py-3 sm:pt-3 sm:pb-4 flex flex-col justify-start">
        <div className="flex justify-between items-center text-[11px] text-dim mb-2 sm:mb-[15px]">
          ...
          {quoteCount > 0 && <span className="bg-surface2 px-1.5 py-0.5 rounded inline-flex items-center gap-1"><IconCollection size={11} />{quoteCount}</span>}
        </div>
        <div className="text-[15px] sm:text-base font-semibold mb-1 leading-[1.3] flex items-center line-clamp-2">{book.title}{book.isPrivate && <span className="text-dim ml-1 inline-flex items-center"><IconLock size={12} /></span>}</div>
