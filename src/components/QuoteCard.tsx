...
const BTN_SMALL_DANGER =
  'bg-transparent text-danger border border-border px-2.5 py-1 rounded-lg text-xs sm:text-[13px] cursor-pointer transition-colors duration-200 font-sans hover:bg-dangersoft'
...
export default function QuoteCard({ quote, book, onBookClick, onEdit, onDelete }: Props) {
  return (
    <div className="bg-surface border border-border rounded-[10px] px-4 py-3.5 sm:px-[22px] sm:py-5 transition-all duration-150 shadow-[inset_3px_0_0_var(--accent)] hover:border-accent">
      <div className="font-serif text-sm sm:text-[15px] leading-[1.7] mb-3 text-ink">
        &ldquo;
        <HighlightedText text={quote.text} highlights={quote.highlights} />
        &rdquo;
      </div>
      {quote.note && (
        <div className="text-xs sm:text-[13px] text-dim mb-2.5 italic pl-3 border-l-2 border-border">{quote.note}</div>
      )}
