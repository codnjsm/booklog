      ...
      <div className="bg-surface border border-border rounded-[14px] px-8 py-10 max-w-[380px] w-full text-center shadow-card">
        ...
        {inApp ? (
          <div className="bg-surface2 border border-border rounded-lg p-4 mb-2 text-sm text-ink leading-[1.6] text-center">
            ...
            <div className="mt-3 bg-bg border border-border rounded-md px-3.5 py-2.5 font-mono text-[13px] text-accent break-all">reading-notes-6935e.web.app</div>
          </div>
        ) : loading ? (
          ...
          <button className="w-full inline-flex items-center justify-center gap-2.5 bg-white text-[#3c4043] border border-[#dadce0] px-[18px] py-[11px] rounded-lg text-sm font-medium cursor-pointer transition-all duration-150 font-sans hover:bg-[#f8f9fa] hover:shadow-[0_1px_3px_rgba(60,64,67,0.15)]" onClick={handleSignIn}>
        ...
        <p className="mt-5 text-xs text-dim leading-[1.6]">로그인하지 않아도 이 브라우저에서만 사용할 수 있어요 · <a className="text-accent cursor-pointer no-underline hover:underline" onClick={onDismiss}>나중에</a></p>
      </div>
    </div>
  )
}
