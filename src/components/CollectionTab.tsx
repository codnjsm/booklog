...
export default function CollectionTab({ quotes, books, words, onDeleteQuote, onAddWord, onDeleteWord }: Props) {
  ...
  return (
    <div>
      <PageHeader title="모음">
        {view === 'quotes' && (
          <button onClick={() => openAddQuote()} className="text-xs sm:text-[13px] font-medium px-3 py-2 sm:px-4 sm:py-2.5 rounded-lg bg-ink text-bg border-none cursor-pointer hover:opacity-90">+ 문장 저장</button>
        )}
      </PageHeader>
