  ...
  return (
    <Modal onClose={onClose}>
      <div className={MODAL_PANEL}>
        <div className={MODAL_HEADER}><h3 className="font-sans text-base font-semibold">책 추가</h3><button className={MODAL_CLOSE} onClick={onClose}>×</button></div>
        <div className={MODAL_BODY}>
          ...
          {error && <div className="text-center py-[30px] px-5 text-dim bg-surface border border-dashed border-border rounded-[10px]"><p>검색 중 오류가 발생했어요.<br />직접 입력해보세요.</p></div>}
          {!loading && results.length > 0 && (
            <div className="max-h-[360px] overflow-y-auto mt-3 border-t border-border pt-3">
              {results.map((item, i) => {
                ...
                return (
                  <div key={i} className="flex gap-3 p-2.5 rounded-lg cursor-pointer transition-colors duration-150 items-start hover:bg-bg" onClick={() => onSelectBook({ title, author, cover: item.thumbnail, year })}>
                    {item.thumbnail
                      ? <img src={item.thumbnail} alt={title} className="w-[50px] h-[70px] object-cover rounded flex-shrink-0 bg-surface2" onError={(e) => { (e.currentTarget as HTMLImageElement).style.display = 'none' }} />
                      : <div className="w-[50px] h-[70px] bg-surface2 rounded flex items-center justify-center text-xl flex-shrink-0">📕</div>
                    }
