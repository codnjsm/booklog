...
export default function BookDetailModal({ bookId, books, quotes, onClose, onEdit, onDelete, onAddQuote, onEditQuote, onTogglePrivate }: Props) {
      ...
      <div className={PANEL}>
        ...
        <div className="p-5 sm:p-6 flex flex-col gap-5">
          ...
          >
            ...
            <span className={`relative w-[38px] h-[22px] rounded-full flex-shrink-0 transition-colors ${isPrivate ? 'bg-accent' : 'bg-border'}`}>
              <span className={`absolute top-[3px] w-4 h-4 rounded-full bg-surface transition-all ${isPrivate ? 'left-[19px]' : 'left-[3px]'}`} />
            </span>
          </button>
          ...
          {book.review ? (
            <div className="flex flex-col gap-2">
              ...
              <div className="bg-surface2 rounded-[10px] px-4 py-3.5 text-sm leading-[1.8] whitespace-pre-wrap">{book.review}</div>
            </div>
          ) : (
            <button onClick={() => onEdit(bookId)} className="flex items-center gap-2.5 px-4 py-3 rounded-[10px] border border-dashed border-border bg-transparent cursor-pointer text-left">
              <span className="text-dim">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M4 20h4L19 9a2.1 2.1 0 0 0-3-3L5 17z" /></svg>
              </span>
          ...
          <div className="flex flex-col gap-3">
            <div className="flex items-center justify-between">
              ...
              {bookQuotes.length > 0 && <button className={BTN_SM} onClick={() => onAddQuote(bookId)}>+ 문장 추가</button>}
            </div>
                ...
                {bookQuotes.map((q) => (
                  <div key={q.id} className="border border-border rounded-[10px] px-4 py-3.5 flex flex-col gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="flex-1" />
                      <button onClick={() => onEditQuote(q.id)} aria-label="문장 편집" className="text-dim hover:text-ink bg-transparent border-none cursor-pointer p-0">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M4 20h4L19 9a2.1 2.1 0 0 0-3-3L5 17z" /></svg>
                      </button>
                    </div>
                  </div>
                ))}
