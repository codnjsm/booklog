...
const FORM_TEXTAREA =
  'w-full bg-bg border border-border text-ink px-3 py-2 rounded-[7px] text-base font-sans placeholder:text-dim placeholder:opacity-50 focus:outline-none focus:border-accent resize-y min-h-[90px] leading-[1.6]'
const BTN =
...
export default function PublishPostModal({ books, quotes, onClose, onPublish, onPublished }: Props) {
  ...
  const publishMutation = useMutation({
    ...
    onError: (err) => showToast(err instanceof Error ? err.message : '발행 중 오류가 발생했어요'),
  })
      ...
      <Modal onClose={onClose}>
        <div className={MODAL_PANEL}>
          <div className={MODAL_HEADER}>
            <h3 className="font-sans text-base font-semibold">발행하기</h3>
          ...
          <div className={MODAL_BODY}>
            ...
            {kindFilter === 'quote' ? (
              pickableQuotes.length === 0 ? (
                <p className="text-sm text-dim text-center py-8">발행할 수 있는 문장이 없어요</p>
              ) : (
              ...
              <p className="text-sm text-dim text-center py-8">발행할 수 있는 책이 없어요</p>
            ) : (
              <div className="flex flex-col gap-2">
                {pickableBooks.map((b) => (
                  <button
      ...
      <div className={MODAL_PANEL}>
        ...
        <div className={MODAL_BODY}>
          ...
          <textarea
            ...
            placeholder="이걸 왜 나누고 싶은지 한마디 적어주세요"
