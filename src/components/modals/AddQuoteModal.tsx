...
interface Entry { text: string; note: string; highlights?: Quote['highlights'] }
...
export default function AddQuoteModal({ books, quotes, bookId, editId, onClose, onSave, onDelete }: Props) {
  ...
  const [entries, setEntries] = useState<Entry[]>(
    existing ? [{ text: existing.text, note: existing.note ?? '', highlights: existing.highlights }] : [{ text: '', note: '' }]
  )
  ...
  const markHighlight = (i: number) => {
    ...
    if (start === end) { showToast('형광펜을 칠할 부분을 먼저 드래그해주세요'); return }
  ...
  const handleSave = () => {
    ...
    valid.forEach((e) => {
      ...
      onSave({ bookId: selectedBookId || null, text, page: '', tags: [], note: e.note.trim(), highlights: shifted.length ? shifted : undefined }, editId)
    })
  }
            ...
            <div key={i} className="bg-surface2 border border-border rounded-[10px] px-3.5 py-3 mb-2.5 relative">
              ...
              <div className="mb-2">
                ...
                <div className="flex items-center gap-2 mt-1.5">
                  ...
                  >
                    <span className="w-3 h-2 rounded-sm bg-highlight" />형광펜
                  </button>
                  {entry.highlights?.length
                    ? <>
                        <span className="font-mono text-[11px] text-dim">{entry.highlights.length}곳</span>
                        <button type="button" onClick={() => clearHighlights(i)} className="text-xs text-dim bg-transparent border-none cursor-pointer p-0 hover:text-ink">모두 지우기</button>
                      </>
