...
const ROW = "w-full flex items-center gap-3 px-4 py-3.5 bg-surface border border-border rounded-xl text-left cursor-pointer"
      ...
      <div className="flex flex-col gap-3">
        {user ? (
          <div className="flex items-center gap-3 px-4 py-3.5 bg-surface border border-border rounded-xl">
            {user.photoURL
              ...
              : <span className="w-11 h-11 rounded-full bg-accent text-white flex items-center justify-center text-[17px] font-semibold flex-shrink-0">{(user.displayName || user.email || '?')[0].toUpperCase()}</span>}
          ...
          <button className={ROW} onClick={onSignIn}>
            <span className="w-11 h-11 rounded-full bg-surface2 text-dim flex items-center justify-center flex-shrink-0"><IconFriends size={20} /></span>
        ...
        <div className="flex flex-col gap-2">
          <button className={ROW} onClick={() => changeTab('friends')}>
            <span className="text-dim"><IconFriends size={19} /></span>
            ...
            <span className="text-dim"><IconChevronRight /></span>
          </button>

          <button className={ROW} onClick={onExport}>
            <span className="text-dim"><IconExport size={19} /></span>
          ...
          {user && (
            <button className={ROW} onClick={onSignOut}>
              <span className="text-dim"><IconSignOut size={19} /></span>
        ...
        <div className="px-1 pt-1 flex flex-col gap-1">
          ...
          <div className="text-[11px] text-dim opacity-70 leading-relaxed">로그아웃해도 이 기기의 기록은 남아 있어요.</div>
        </div>
      </div>
    </div>
  )
