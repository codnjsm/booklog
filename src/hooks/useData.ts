...
function makeSeedData(): AppState {
  ...
  return {
    books: [
      { id: id1, title: '아주 작은 습관의 힘', author: '제임스 클리어', year: '2019', status: 'done', rating: 5, review: '1%의 작은 변화가 쌓이면 결국 거대한 차이를 만든다는 메시지가 인상 깊었다. 시스템이 목표를 이긴다는 관점.', cover: '', createdAt: now, finishedAt: now },
      { id: id2, title: '미움받을 용기', author: '기시미 이치로, 고가 후미타케', year: '2014', status: 'reading', rating: 0, review: '', cover: '', createdAt: now },
    ],
    quotes: [
      { id: uid(), bookId: id1, text: '당신은 목표 수준으로 떨어지는 것이 아니라, 시스템 수준으로 떨어진다.', page: '47', tags: ['습관', '성장'], note: '결심보다 환경 설계가 중요하다는 걸 다시 한 번.', createdAt: now },
      { id: uid(), bookId: id2, text: '인생은 누군가 정해주는 것이 아니라 스스로 선택하는 것이다.', page: '', tags: ['인생', '용기'], note: '', createdAt: now },
    ],
    words: [],
  }
}
...
export function useData(user: User | null) {
  ...
  useEffect(() => {
    ...
    loadUserData(user.uid).then((cloud) => {
      if (cloud?.books) {
  ...
  const updateBook = useCallback((id: string, patch: Partial<Book>) => {
    update((s) => ({
      ...
      books: s.books.map((b) => {
        if (b.id !== id) return b
        const next = { ...b, ...patch }
        if ((patch.status === 'reading' || patch.status === 'done') && !next.startedAt) next.startedAt = new Date().toISOString()
        if (b.status !== 'done' && patch.status === 'done' && !patch.finishedAt) next.finishedAt = new Date().toISOString()
        return next
      }),
    }))
  }, [update])
  ...
  const addQuote = useCallback((data: Omit<Quote, 'id' | 'createdAt'>) => {
  ...
  const addWord = useCallback((data: Omit<Word, 'id' | 'createdAt'>) => {
  ...
  const exportData = useCallback(() => {
    ...
    if (doneBooks.length > 0) { lines.push('─── 다 읽은 책 ───'); lines.push(''); writeBooks(doneBooks) }
    if (readingBooks.length > 0) { lines.push('─── 읽는 중 ───'); lines.push(''); writeBooks(readingBooks) }
    if (wishBooks.length > 0) { lines.push('─── 읽고 싶은 책 ───'); lines.push(''); writeBooks(wishBooks) }
  ...
  return { state, syncStatus, addBook, updateBook, deleteBook, addQuote, updateQuote, deleteQuote, addWord, deleteWord, exportData, setGoal }
}
